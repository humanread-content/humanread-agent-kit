...
SERVER_INSTRUCTIONS += """
Treat a book social card as a required first-review decision for every novel and translation edition. When image generation is available, proactively design and show a 1200x630 candidate; upload only after the author approves that exact image and confirms publication rights. If declined, record the explicit portrait-cover fallback choice. Never silently skip it. A portrait fallback produces only a compact social summary, not a large X image. Translation editions upload their own cards; draft cards remain invisible to social crawlers until that edition is reviewed and published."""
SERVER_INSTRUCTIONS += """
