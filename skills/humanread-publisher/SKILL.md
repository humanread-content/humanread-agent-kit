...
3. Preserve prose exactly unless the author explicitly requests editing. For a linked translation, wait for its initial Git sync, then call `sync_translation_assets` before uploading translated `sandbox_html`. Reuse only the returned same-hash assets from the bound immutable source version; never re-upload those images one by one or copy an image from a newer draft. New translations also inherit these assets automatically during initial sync, so the tool may report them as already present.
...
5. Run `create_novel`, then `upload_chapter` in reading order.
   For illustrations, wait for Git sync, run `upload_image_asset` with meaningful alt text, and insert only its returned Markdown/HTML or exact sandbox asset reference in that same novel. When connected to the hosted/remote MCP that cannot read the Agent's filesystem, base64-encode the local PNG, JPEG, or WebP and call `upload_image_asset_base64` instead. Never upload SVG, animation, tracking pixels, secrets, or unrelated files.
   ...
   Treat the book social card as a required first-review decision for every novel and every translation edition. Proactively design a 1200 × 630 candidate when image generation is available, show the exact candidate, and ask for image approval plus publication-rights confirmation. Upload it only after both confirmations. If the author declines, record their explicit choice to use the portrait-cover fallback and continue; do not silently skip the decision. Without a book or chapter card, Humanread falls back to the portrait cover and social platforms receive a compact `summary` card. Never promise a large X/social image from that fallback. To obtain a large landscape card, upload and publish a book card, or a chapter card for that exact chapter. Translation editions are separate publications and must upload their own cards to their own novel repo; source-edition cards are not silently reused. Social crawlers see only the latest published immutable snapshot, not an unreviewed draft asset.
...
12. API-key creation records the author's standing material-rights confirmation, public-copy acknowledgement, and default reader license. Call `get_publication_defaults` when the effective default needs to be shown or verified. Omit `reader_license` for normal publication so the API inherits that saved default without asking again. If standing authorization is false, tell the author to rotate the legacy key in Author Studio; never guess. If the author requests a different license, explain that `all-rights-reserved` prohibits reuse beyond applicable law, `CC-BY-4.0` permits reuse with attribution, and `CC-BY-SA-4.0` also requires adaptations to use the same license; obtain the exact choice and pass it as `reader_license`. Never change it based on Agent preference.
...
- A novel, chapter, private prompt, and author conversation are not reusable design knowledge. Never copy them into a public recipe or embedding index merely because they demonstrate a useful effect.
- Call `get_knowledge_base_mode`; when it is `private` or `public`, call `save_design_recipe` after successful non-trivial design work without interrupting for per-recipe approval. When it is `disabled`, do not extract or upload knowledge.
- Call `search_design_knowledge` for non-trivial presentation work using the author's intent, language, motion preference, and accessibility needs. Search is multilingual: the requested language is a ranking preference, not a reason to exclude a useful recipe saved in another language. Fetch a selected immutable version with `get_design_recipe`.
- Tell the author which recipe and immutable version will be used. Adapt it to the work; never paste story text, named fictional entities, tracking code, remote resources, or unsupported markup from an example.
- Extract only bounded instructions, a minimal synthetic example, constraints, accessibility behavior, and provenance IDs. If safe extraction is impossible without revealing story content or third-party material, skip it.
- Visibility is account-wide. Never invent a per-recipe visibility override. Humanread generates embeddings locally; never calculate or upload a vector.
